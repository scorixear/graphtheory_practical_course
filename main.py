...
def main():
    ...
    smiles_to_crn.run(smiles_list_directory, rdir_crn)
    ...
    input("< Press ENTER to continue...")
    aa_synthesis_vis.run(rdir_crn, rdir_amino_reaction_cycle, rdir_aa_synthesis_vis, input_directory)
    ...
    #rfile_extract_kegg_pathways = rdir_extract_kegg_pathways + "pathways.json"
    #mkdir(rdir_extract_kegg_pathways)
    #print("\n==== Kegg pathways extraction ====")
    #print("> Output will be under "+rfile_extract_kegg_pathways)
    #input("< Press ENTER to continue...")
    #extract_kegg_pathways.run(rfile_extract_kegg_pathways)
    #print("> Kegg pathways extraction successfull")

    # ------------------------------------------------------------------------------
    # Enrichment test
    enrichment_test = __import__("05_enrichment_test")
    rdir_enrichment_test= data_directory+"12_enrichment_test/"
    rfile_extract_kegg_pathways = input_directory+"03_kegg_pathways/pathways.json"
    ...
    print("! Please provide the Kegg pathways.json under "+rfile_extract_kegg_pathways)
    input("< Press ENTER to continue...")
    enrichment_test.run(rdir_pulp_solve, rfile_extract_kegg_pathways, rdir_enrichment_test, input_directory)
    print("> Enrichment test successfull")

    # ------------------------------------------------------------------------------
    # generate statistics about ATN
    atm_analysis = __import__("01_atn_analysis")
    graph_dir: str = input_directory+"04_atn_graphs/"
    rdir_atm_analysis: str = data_directory+"13_atn_analysis/"
    ...
    print("  > Molecule Start Atom Number: "+str(molecule_start_atom))
