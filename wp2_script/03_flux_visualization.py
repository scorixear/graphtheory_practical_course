...
fluxDir = "data/flux_results/"
# aaCycleDir = "data/amino_reaction_cycle/"
...
def read_file(file: str, fluxDir: str = "data/flux_results/", aaCycleDir: str = "data/amino_reaction_cycle/") -> tuple([dict(), nx.DiGraph()]):
...
for file in os.listdir(fluxDir):
    ...
    reactionNodes = [node for node in aaSynthesisSubgraph if aaSynthesisSubgraph.nodes(
        data=True)[node]["nodeType"] == 1]
    ...
    nx.draw(reactionSubgraph, pos=nx.shell_layout(reactionSubgraph),
            with_labels=True, node_size=20, font_size=4, width=0.3, arrowsize=5, node_color=[flux[node] for node in reactionSubgraph], cmap=plt.cm.viridis)
