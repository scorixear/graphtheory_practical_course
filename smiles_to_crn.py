...
def parseLines(lines):
    ...
    # parse reaction
    reactionData["in"] = [line.replace(" ", "")
                          for line in lines[2].split("=")[0].split(" + ")]
    reactionData["out"] = [line.replace(
        " ", "") for line in lines[2].split("=")[1].split(" + ")]
            ...
            if line.startswith("Bigg ID"):
                ...
                for pre in reactionData["in_multiplicity"].keys():
                    ...
                    if reactionData["reversible"]:
                        G.add_edge(reactionID, pre, multiplicity=reactionData["in_multiplicity"][pre],
                                   direction=2)
               ...
               # add the products as succsessor nodes to the reaction
                for succ in reactionData["out_multiplicity"].keys():
                    ...
                    if reactionData["reversible"]:
                        G.add_edge(succ, reactionID, multiplicity=reactionData["in_multiplicity"][succ],
                                   direction=2)
