...
def build_reaction_graph(G: nx.DiGraph,source:str, acids_present: list[str], common_compounds: list[str]) -> nx.DiGraph:
...
def enumerate_simple_paths(reactionGraph: nx.DiGraph(), source: str, target: str, maxPaths: int) -> list[str]:
...
#extract all paths of equal size
    ...
    for pathLength in paths_equal_length:
        ...
        print(f"für AS {aa} haben die Pfade der Länge {pathLength} {len(sharedPath)} gemeinsame Reaktionen")
