...
def run(species_parent_dir: str = "data/atn_graphs/"):
...
def get_cycle_lengths(graph: nx.DiGraph) -> dict[int, list]:
    ...
    for i, conn_comp in enumerate(nx.connected_components(new_graph)):
        cycles[i] = nx.cycle_basis(
            new_graph.subgraph(conn_comp)
        )  # all_cycles.find_all_cycles(new_graph.subgraph(conn_comp))
