...
constrains_list = [-1, -2, -5, -10, -20, -50, -100, -200, -400, -600, -800, -1000]
...
def create_compound_constrains(essential_compounds: list[str], constraint: int) -> dict[str, int]:
    ...
    essential_compounds_constrains = {es : constraint for es in essential_compounds}
...
def main():
    ...
    essential_compounds = pulp_solve.read_file("wp1_script/essential_compounds.txt")   

    results = {"constrain": [], "biomass": {}, "activated_reactions": {}}
    ...
    for constrain in constrains_list:
        ...
        for entry in os.scandir(datadir):
            if entry.is_file() and entry.name.endswith(".pi"):
                ...
                pulp_solve.add_exchange_reactions(graph, essential_compounds + ['D-glucose'])

                variables: dict[str, pulp.LpVariable] = pulp_solve.generate_variables(graph, essential_compounds_constrains)
